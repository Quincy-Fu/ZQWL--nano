...
CONFIG = {
    ...
    "ransac_max_points": 1400,
    "ransac_max_circles": 6,
    ...
    "enable_radial_scan": True,
    "radial_trigger_confidence": 0.45,
    "radial_center_search_px": 150,
    "radial_center_step_px": 14,
    ...
    "preview_detect_interval_s": 0.12,
    "preview_result_max_age_s": 0.60,
...
def _extract_ransac_candidates(dark):
    """从二值 mask 边缘中反复 RANSAC，提取多个圆环候选。"""
    edges = cv2.Canny(dark, CONFIG["ransac_edge_low"], CONFIG["ransac_edge_high"])
    ...
    for _ in range(int(CONFIG["ransac_max_circles"])):
        cand, _ = _ransac_one_circle(remaining, rng, dark.shape)
        if cand is None:
            break
...
def find_ring_center(frame, dark=None):
    """返回同心圆中心和自动比例估算结果。
    ...
    if need_ransac:
        ransac_candidates, _ = _extract_ransac_candidates(dark)
    ...
    if need_radial:
        radial_best = _radial_scan_result(dark, seed_candidates=all_candidates)
...
def _move_by_offset(offset, verbose=True):
    """按检测得到的 dx/dy 微调；定位完成后沿 dy 正方向前进固定距离。"""
    align_ok = True
    if abs(offset["dx_px"]) <= CONFIG["dead_zone_px"] and \
       abs(offset["dy_px"]) <= CONFIG["dead_zone_px"]:
        ...
        align_ok = _safe_fine_move(offset["dx_mm"], offset["dy_mm"],
...
def _setup_debug_trackbars():
    ...
    cv2.createTrackbar("black", win, int(CONFIG["black_thresh"]), 255, _noop_trackbar)
...
def _read_debug_trackbars():
    ...
    win = CONFIG["trackbar_window_name"]
    try:
        CONFIG["black_thresh"] = max(1, cv2.getTrackbarPos("black", win))
...
def preview():
    ...
    print("[ring] 预览已启动: q退出, p打印当前dx/dy, A微调后dy前进100mm")
    last_result = None
    last_candidates = []
    ...
    while True:
        ...
        if frame is None:
            ...
            continue
        now = time.time()
        mask = _threshold_dark(frame)
        result = None
        candidates = last_candidates
        if now >= next_detect_time:
            result = find_ring_center(frame, dark=mask)
        ...
        if key in (ord('a'), ord('A')):
            ...
            fresh_mask = _threshold_dark(frame)
            fresh_result = find_ring_center(frame, dark=fresh_mask)
