...
# 导航命令 (偶数=命令, 奇数=响应)
TYPE_CMD_GOTO         = 0x10  # 8B=x/y; 12/16B=x/y/yaw[/speed] 平滑移动转角
...
class SerialComm:
    def __init__(self, port=None, baudrate: int = 115200, timeout: float = 0.1):
        ...
        self._last_rotate_write_ts: float = 0.0                   # 转盘帧之间留短间隔，降低串口/CAN侧丢帧概率
        self._reset_parser()
    ...
    def _write_rotate_frame(self, frame: bytes, min_gap_s: float = 0.06) -> None:
        """发送一帧转盘命令；连续转盘帧之间留短间隔。"""
        now = time.monotonic()
        wait_s = float(min_gap_s) - (now - self._last_rotate_write_ts)
        if wait_s > 0.0:
            time.sleep(wait_s)
        self._ser.write(frame)
        self._last_rotate_write_ts = time.monotonic()

    def send_rotate(self, pos: int, repeats: int = 1, repeat_delay_s: float = 0.08) -> int:
        if not self._ser:
            raise RuntimeError("serial not started")
        ...
        if pos == 0 and repeat_n > 1:
            # 下位机收到 slot0 会执行回零；重复回零会拖慢流程，所以 0 槽不做双发。
            repeat_n = 1
        for idx in range(repeat_n):
            self._write_rotate_frame(frame)
            if idx + 1 < repeat_n and repeat_delay_s > 0.0:
                time.sleep(float(repeat_delay_s))
        return seen
    ...
    def send_goto_yaw(self, x: float, y: float, yaw_deg: float,
                      speed: float | None = None) -> None:
        """发送平滑走点+转角命令。

        复用 TYPE_CMD_GOTO: 8B仍是旧GOTO，12/16B由新固件解释为 GOTO_YAW。
        speed=None 时下位机使用 GOTO_DEFAULT_SPEED。
        """
        self._check_finite(x, y, yaw_deg)
        if speed is None:
            self._send_nav(TYPE_CMD_GOTO, struct.pack("<fff", x, y, yaw_deg))
            return
        self._check_finite(speed)
        if speed <= 0.0:
            raise ValueError(f"goto_yaw speed must be > 0, got {speed}")
        self._send_nav(TYPE_CMD_GOTO, struct.pack("<ffff", x, y, yaw_deg, speed))
    ...
    def wait_status_after(self, expect: int, seen_seq: int, timeout: float) -> int | None:
        """等待 seen_seq 之后的指定响应，返回原始 status；超时返回 None。"""
        deadline = time.monotonic() + timeout
        seen = int(seen_seq)
        with self._resp_cond:
            while True:
    ...
    def goto_yaw(self, x: float, y: float, yaw_deg: float,
                 speed: float | None = None,
                 timeout: float = 40.0) -> bool:
        """单条下位机命令：移动到(x,y)的同时平滑转到yaw_deg。"""
        self.send_goto_yaw(x, y, yaw_deg, speed)
        return self.wait_for(TYPE_CMD_GOTO_RESP, timeout)
    ...
    def rotate(self, pos: int, timeout: float = 10.0,
        ...
        如果本次 ROTATE_RESP 超时/失败, 会重发同一目标；转盘使用绝对位置，重复同一目标是幂等的。
        """
        attempts = max(1, int(retries) + 1)
        for attempt in range(attempts):
            seen = self.send_rotate(pos)          # 越界抛 ValueError
            if self.wait_for_after(TYPE_ROTATE_RESP, seen, timeout):
                return True
            if attempt + 1 < attempts:
                print(f"[comm] ROTATE {pos} 未确认，{retry_delay_s:.2f}s 后重发 ({attempt + 2}/{attempts})")
...
def is_started() -> bool:
    """判断串口是否已初始化并保持打开。"""
    return _comm is not None and _comm.is_started()
...
def send_rotate(pos: int, repeats: int = 1, repeat_delay_s: float = 0.08) -> int:
    if _comm is None:
        raise RuntimeError("serial not initialized, call init() first")
    return _comm.send_rotate(pos, repeats=repeats, repeat_delay_s=repeat_delay_s)
...
def send_goto_yaw(x: float, y: float, yaw_deg: float,
                  speed: float | None = None) -> None:
    if _comm is None:
        raise RuntimeError("serial not initialized")
    _comm.send_goto_yaw(x, y, yaw_deg, speed)
...
def wait_status_after(expect: int, seen_seq: int, timeout: float = 30.0) -> int | None:
    """等待 seen_seq 之后的指定响应；返回 status，超时返回 None。"""
    if _comm is None:
        raise RuntimeError("serial not initialized, call init() first")
    return _comm.wait_status_after(expect, seen_seq, timeout)
...
def goto_yaw(x: float, y: float, yaw_deg: float,
             speed: float | None = None,
             timeout: float = 40.0) -> bool:
    if _comm is None:
        raise RuntimeError("serial not initialized, call init() first")
    return _comm.goto_yaw(x, y, yaw_deg, speed, timeout)
...
def rotate(pos: int, timeout: float = 10.0, retries: int = 1, retry_delay_s: float = 0.20) -> bool:
    if _comm is None:
        raise RuntimeError("serial not initialized, call init() first")
    return _comm.rotate(pos, timeout, retries=retries, retry_delay_s=retry_delay_s)
